...
public class XmlUtil {
    ...
    public String AppendValue(String nodeName, String sonNdeName, int[] idArr) {
        ...
        return AppendValue(nodeName, sonNdeName, strArr);
    }
    ...
    public String AppendValue(String nodeName, String sonNdeName, String[] valueArr) {
        ...
        for (int i = 0; i < valueArr.length; i++)
            strBuilder.append("     <" + sonNdeName + " value=\"" + valueArr[i] + "\" />");
    ...
    public String AppendValue(String nodeName, List<TowerPart> partList) {
        if (partList == null || partList.isEmpty())
            return "";
        StringBuilder strBuilder = new StringBuilder();
        strBuilder.append("     <" + nodeName + " count=\"" + partList.size() + "\">");
        for (int i = 0; i < partList.size(); i++)
            strBuilder.append("     <Part id=\"" + partList.get(i).getId() + "\" md5= \"" + partList.get(i).getMd5() + "\" />");
        strBuilder.append("     </" + nodeName + ">");
        String str = strBuilder.toString();
        queryPropList.add(str);
        return str;
